...
{
    extern std::string _kinect_ns;
    static tf::TransformBroadcaster tilt_broadcaster;
    tf::Transform transform;
    transform.setOrigin(tf::Vector3(0,0,0));
    transform.setRotation(tf::Quaternion(0,- msg.current_pos - _p_offset,0));
    tilt_broadcaster.sendTransform(tf::StampedTransform( transform, ros::Time::now(), "/tilt_link", _kinect_ns+"_link"));
/*
        tf::StampedTransform(
            tf::Transform( tf::Quaternion(1, 1, 1, 1),
...
int main(int argc, char** argv) {
    ...
    n.param("kinect_ns", _kinect_ns, std::string(""));
